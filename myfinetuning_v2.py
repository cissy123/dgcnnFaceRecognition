...
TRAIN_FILES = provider.getDataFiles( \
    os.path.join(BASE_DIR, 'data/bu3d/train_files.txt'))
TEST_FILES = provider.getDataFiles( \
    os.path.join(BASE_DIR, 'data/bu3d/test_files.txt'))
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:' + str(GPU_INDEX)):
            ...
            pretrained_variable = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='get_model')
            saver_restore_first_stage = tf.train.Saver(pretrained_variable)
        ...
        saver_restore_first_stage.restore(sess, 'log/model.ckpt')
        ...
        for epoch in range(MAX_EPOCH):
            ...
            if epoch % 10 == 0:
                save_path = saver.save(sess, os.path.join(LOG_DIR, "model-fine-tuning.ckpt"))
