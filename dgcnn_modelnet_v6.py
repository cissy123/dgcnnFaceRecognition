...
def get_model(point_cloud, is_training, bn_decay=None):
    ...
    # X_4 = tf.reshape(X_4, [batch_size, num_point, 1])
    # X_4 = tf.tile(X_4, [1, 1, num_point])
    # X_4 = tf.sigmoid(X_4)
    # net = tf.squeeze(net)
    # net = tf.matmul(X_4, net)
    # net = tf.reshape(net, [batch_size, num_point, 1, -1])


    #################################
    #0716 modify reference to SGPN
    X_4 = tf_util.conv2d(net, 1, [1, 1],
                         padding='VALID', stride=[1, 1],
                         activation_fn=None,
                         scope='X_4')
    X_4 = tf.reshape(X_4, [batch_size, num_point, 1])
    X_4 = tf.tile(X_4, [1, 1, 1])
    X_4 = tf.nn.sigmoid(X_4)
    net = tf.squeeze(net)
    net = tf.multiply(X_4, net)
    net = tf.reshape(net, [batch_size, num_point, 1, -1])
